...
Repository-level records are published only after GitHub's live API confirms
that the repository is public. A stale registry can therefore never make a
private repository eligible for Pages output.
"""
...
def live_visibility(repository: str) -> str:
    req = urllib.request.Request(
        ...
        headers={"Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "repo-auditor-public-bundle"},
    )
...
def build(registry_path: Path = REGISTRY, audits_dir: Path = AUDITS, out_dir: Path = OUT, visibility_resolver: Callable[[str], str] = live_visibility) -> dict[str, Any]:
    ...
    for item in repos:
        ...
        repository = f"{owner}/{item['name']}"
        if visibility_resolver(repository) != "public":
            excluded_count += 1
            continue
    ...
    manifest = {
        ...
        "repository_count": len(safe_repos),
        "live_visibility_exclusions": excluded_count,
        "audit_sidecars": sorted(copied),
        "private_repository_metadata_included": False,
        "visibility_policy": "live-github-fail-closed",
    }
