...
def compact_evidence(e):
    common=e.get('common_files') or {}
    runs=e.get('head_workflow_runs') or {}
    ...
    return {
        ...
        'unpinned_action_refs':len((e.get('action_supply_chain') or {}).get('unpinned') or []),
...
def public_summary(owner,records,failures,expected_total=None,expected_private=None,private_requested=False):
    ...
    return {'schema_version':2,'scope':'privacy-preserving-account-audit-summary','owner':owner,'generated_at':datetime.now(timezone.utc).isoformat(),'audit_status':'PASS' if complete else 'PARTIAL','inventory':{'observed_total':observed,'public':len(public),'private':len(private),'scan_failures':len(failures),'expected_total':expected_total,'expected_private':expected_private,'private_names_published':False},'coverage':{'status':'complete' if complete else 'partial','reasons':reasons,'evidence_collected':len(records),'readme_present':sum(r['readme'] for r in records),'license_present':sum(r['license'] for r in records),'security_policy_present':sum(r['security_policy'] for r in records),'repositories_with_workflows':sum(r['workflow_count']>0 for r in records),'repositories_with_unpinned_actions':sum(r['unpinned_action_refs']>0 for r in records),'head_ci_green':sum(r['head_ci_green'] for r in records),'ai_ready':sum(r.get('ai_readiness_state')=='AI_READY' for r in records),'agents_present':sum(r.get('agents',False) for r in records),'handoff_present':sum(r.get('handoff',False) for r in records),'continuity_full':sum(r.get('continuity_full',False) for r in records)},'privacy':{'private_details_location':'private-evidence/portfolio-scan.json (gitignored; never Pages)','pages_contains_private_details':False},'self_audit':{'included':any(r['repository']==f'{owner}/repo-auditor' for r in records),'repository':'repo-auditor'}}
