...
def scan(owner,allow_private,expected_total=None,expected_private=None):
    ...
    private_report={'schema_version':2,'scope':'authenticated-account-audit','owner':owner,'generated_at':datetime.now(timezone.utc).isoformat(),'repositories':records,'failures':failures}
    return private_report,public_summary(owner,records,failures,expected_total,expected_private,allow_private)
...
def main():
    ...
    if a.require_complete and summary['coverage']['status']!='complete': return 2
    return 0
